...
def get_data_from_hh(key_word, area) -> list:
    # словарь с данными по вакансиям
    ret_vacancy_list = []
    ...
    for page in range(20):
        params['page'] = page
        page_obj = requests.get(url, params)
        # проверяем что сервер отвечает
        if page_obj.status_code < 200 or page_obj.status_code > 299:
            print('Cannot get actual currency! HTTP response code: %d'%page_obj.status_code)
            quit()
...
def get_salary_and_vacancies(data_list) -> tuple:
...
def save_data_to_json(pages, name) -> None:
    ...
    with open('./docs/%s.jsonc'%name, 'w+', encoding='utf-8') as json_file:
...
def getDataFromJson(name) -> list:
    ...
    with open('./docs/%s.jsonc'%name, encoding='utf-8') as json_file:
...
for technology in technologies:
    ...
    # загрузка с json
    vacancy_list = getDataFromJson(technology)
    ...
    if number_of_vacancies != 0:
        ...
        print('a/an {0} developer has nothing to do'.format(technology))
